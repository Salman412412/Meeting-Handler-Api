...
# http://127.0.0.1:5000/api/v1/meeting?id=139016136604805407078985976850150049467,170378154979885419149243073079764064027&length=35&earliest_date=2-19-2014&earliest_time=10:30%20AM&latest_date=3-18-2014&latest_time=09:30%20AM&office_hours=08-12
...
def is_free_time(start_time, duration, busy_time_start, busy_time_end):
    if ((start_time + timedelta(minutes=duration)) <= busy_time_start) or (start_time >= busy_time_end):
        return True
    else:
        return False
...
def api_id():
    ...
    if 'id' not in request.args:
        ...
        employeeids = request.args['id'].split(',')
        ...
        for employeeid in employeeids:
            ...
            for line in lines:
                ...
                if line.split(';')[0] == employeeid:
                    ...
                    if is_free_time(suggested_time, (30 * length), busy_time_start, busy_time_end):
                        # Checks if the suggested time is in the office hours
                        if (int(str(suggested_time.strftime('%H'))) >= office_hours_start) and (
                                int(str(
                                    (suggested_time + timedelta(minutes=30)).strftime('%H'))) <= office_hours_end) and (
                                int(str(
                                    (suggested_time + timedelta(minutes=30)).strftime('%H'))) >= office_hours_start):
                            previous_employee = True
                            break
            if not previous_employee:
                break

        if employeeid == employeeids[-1]:
            found = True
